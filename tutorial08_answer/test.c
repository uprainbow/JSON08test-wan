...
static int test_count = 0;  //一共测试了多少个测试
static int test_pass = 0;	//通过的测试个数

//传入 equality  expect  actual  format
...
//检验解析的结果或数据类型  传入预期值和实际值  格式: %d
#define EXPECT_EQ_INT(expect, actual) EXPECT_EQ_BASE((expect) == (actual), expect, actual, "%d")

//检验解析number后所得的值    传入预期值和实际值  格式: %.17g
#define EXPECT_EQ_DOUBLE(expect, actual) EXPECT_EQ_BASE((expect) == (actual), expect, actual, "%.17g")

//检验解析string后所得的值    传入预期值 实际值 字符串长度  格式: %s
...
//使用多行的宏来减少重复代码
//通过检验  解析结果是否为预期  数据类型是否正确  解析后所得到实际值是否为预期值
...
//重构  即把相似代码整合到一个函数简化代码 lept_parse_null/false/true  判断过程代码相似
//传入参数 error:解析结果  json实际文本内容
...
//检验生成器 传入json  测试 生成是否与解析值相同   代码重构
...
static void test_access_boolean() {
	...
	//在编写单元测试时，我们故意先把值设为字符串，
	//那么做可以测试设置其他类型时，有没有调用 lept_free() 去释放内存。
	lept_set_string(&v, "a", 1);

	//不使用lept_prass解析函数得到值  而是直接写入创建值进行测试
	//写入true
	lept_set_boolean(&v, 1);
	//读出值类型并检验是否与预期相同
